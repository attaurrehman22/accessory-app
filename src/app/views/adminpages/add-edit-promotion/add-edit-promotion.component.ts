...
export class AddEditPromotionComponent implements OnInit {
  ...
  constructor(
    private fb: FormBuilder,private http:HttpService,private alertService:AlertsServicesService,
    ...
    end_date: data?.data?.end_date
      ...
      if (data?.data?.promotion_banner) {
        this.previewImage ="https://api.chronosouq.com/" + data.data.promotion_banner;
      }
    }
  }
      ...
      if(this.labelMessage == 'Add New Promotion'){
        this.http.AddPromotionProducts(formData).subscribe(
          (res)=>{
            ...
            this.alertService.showAlert('warning','Error in product adding into Promotion')
          }
        )
      }else{
        this.http.EditPromotionProducts(formData,this.promotionForm.value.product_id).subscribe(
          (res)=>{
            this.alertService.showAlert('success','Edit product into promotion')
            this.dialogRef.close(true);
          },(err)=>{
            this.alertService.showAlert('warning','Error in product updating into Promotion')
          }
        )
      }
